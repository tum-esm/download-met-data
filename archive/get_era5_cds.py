...
def getvars(means=False, tm=1, levtype="pl"):
    ...
    sname["WWND"] = [
        ...
        "2",
        "8",
    ]  # units Pa/s. convert to hPa/s for HYSPLIT
    ...
    sname["HGTS"] = [
        ...
        "3",
        "4",
    ]  # units m^2 / s^2. Divide by 9.8m/s^2 to get meters.
    ...
    sname["ZWND"] = [
        ...
        "-1",
        "-1",
    ]  # eta-coordinate vertical velocity units s^-1
    ...
    ]  # units m/s       #Analysis (needed) ERA5
    sname["V10M"] = [
        "10v",
        "166",
        "1.0",
    ...
    sname["PRSS"] = [
        ...
        "134.128",
        "surface_pressure",
    ]  # Instantaneous. Units Pa        #multiplier of 0.01 for hPa.
    sname["TCLD"] = [
        "tcc",
        "164",
        "1.0",
    ...
    sname["CAPE"] = [
        ...
        "59",
        "1.0",
        "59.128",  # Instantaneous. convective potential energy : units  J/kg
    ...
    sname["UMOF"] = [
        ...
        "229",
        "1.0",
        "229",  # units of N/m2  eastward turbulent surface stress
    ...
    sname["VMOF"] = [
        ...
        "230",
        "1.0",
        "230",  # units of N/m2  northward turbulent surface stress
    ...
    sname["TPP1"] = [
        ...
        "228.128",
        "total_precipitation",
    ]  # Accumulated precipitation. units of m. multiplier is 1.
    sname["TPP3"] = [
        "tp",
        "228",
        "1.0",
    ...
    ]  # forecast surface roughnes : units m
    ...
    ]  # units J/m^2 (surface sensible heat flux) (divide by 3600 to get W/m^2)
    sname["LTHF"] = [
        "slhf",
        "147",
        amult,
    ...
    if instant:
        ...
        ]  # instantaneous SHTF. units W/m^2.
    ...
    sname["USTR"] = [
        ...
        "3.228",
        "friction_velocity",
    ]  # units of m/s (multiplier should be 1)
...
parser.add_option(
    "-f", type="int", dest="placeholder", default="1", help="Does not do anything."
)
parser.add_option(
    "--dir",
    type="string",
...
if (
    not (options.retrieve3d)
    and not (options.retrieve2d)
    and not (options.retrieve2da)
    and not (options.retrieve2df)
):
...
if stream not in ["oper", "enda"]:
    print(
        "Warning: stream"
        + options.stream
        + " is not supported. Only oper and enda streams supported"
    )
    sys.exit()
if stream == "enda":
    estr = "enda"
...
for wtime in wtimelist:
    if (
        options.getfullday != 1
        and options.timeperiod != -99
        and options.timeperiod != iii
    ):
